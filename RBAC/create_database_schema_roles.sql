/*
  Required variables:
	...
	IMPORTANT:
	The following script represents a centralized access control model where all databases and schemas are created by SYSADMIN.
	This model works well for databases designed for production workloads with robust security.

	Consider a modified version of this security model for sandbox and/or non-production workloads
	For instance, you may want to have SYSADMIN create the database, but allow custom roles the ability
		to create and manage schemas and the objects they contain.
		Be sure to create separate warehouses for these such that production workloads are not impacted.  
*/
